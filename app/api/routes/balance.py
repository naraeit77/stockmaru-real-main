...
@router.get("/deposit", summary="예수금 조회 (원화 및 외화)")
def read_deposit():
    """
    예수금 조회 API - 원화(국내) 및 외화(해외) 예수금 정보를 함께 반환

    ### 응답
    - 성공 시: 국내 및 해외 예수금 정보 반환
    - 실패 시: 오류 메시지와 함께 HTTP 상태 코드 반환
    ...
    ```json
    {
      ...
      "msg1": "예수금 조회 성공",
      "domestic": {
        "dnca_tot_amt": "0",  // 예수금총액(원화)
        "nxdy_excc_amt": "0",  // 익일정산금액
        "prvs_rcdl_excc_amt": "0",  // 가수도정산금액
        "cma_evlu_amt": "0",  // CMA평가금액
        "tot_evlu_amt": "0"  // 총평가금액
      },
      "overseas": {
        "frcr_ord_psbl_amt1": "0",  // 외화주문가능금액(USD)
        "frcr_dncl_amt_2": "0",  // 외화예수금액(USD)
        "ovrs_ord_psbl_amt": "0"  // 해외주문가능금액(원화환산)
      }
    }
    ```
    """
    try:
        result = get_deposit_info()
        if result.get("rt_cd") != "0":
            raise HTTPException(status_code=500, detail=result.get("msg1", "예수금 조회 중 오류 발생"))
        return result
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"예수금 조회 중 오류 발생: {str(e)}")



# 해외주식 예약주문 접수 요청 모델
class OrderResvRequest(BaseModel):
    pdno: str = "AAPL"  # 종목 코드 (예: AAPL)
    ovrs_excg_cd: str = "NASD"  # 거래소 코드 (예: NASD - 나스닥)
    ft_ord_qty: str = "1"  # 주문 수량 (예: 1)
    ft_ord_unpr3: str = "180.00"  # 주문 단가 (예: 180.00)
...
def order_resv_route(order: OrderResvRequest):
    ...
    ⚠️ **미국 예약주문 접수 가능 시간 (한국시간 기준)**
    - **평상시**: 10:00 ~ 23:20 (주문제한: 16:30~16:45 시스템 정산)
    - **서머타임**: 10:00 ~ 22:20 (주문제한: 16:30~16:45 시스템 정산)
    - **주문 전송**: 23:30 정규장 전송 (서머타임 시 22:30)
    - **미국 거래소 운영**: 23:30~06:00 (서머타임 22:30~05:00)

    ⚠️ **현재 시간이 접수 가능 시간대인지 확인하세요!**

    ### 입력값 설명
    - **pdno**: 종목 코드 (예: AAPL - 애플 주식)
    - **ovrs_excg_cd**: 거래소 코드 (예: NASD - 나스닥, NYSE - 뉴욕증권거래소)
    - **ft_ord_qty**: 주문 수량 (예: 1 - 1주 매수)
    - **ft_ord_unpr3**: 주문 단가 (예: 180.00 - 달러 단위로 소수점 2자리까지)
    ...
    try:
        ...
        # 예약주문 가능 시간대 체크 (미국 주식의 경우)
        if order.ovrs_excg_cd in ["NASD", "NYSE", "AMEX"]:
            # 10:00 ~ 23:20 (16:30~16:45 제외)
            is_valid_time = False
            ...
            # 시스템 정산 시간 체크
            if current_hour == 16 and 30 <= current_minute <= 45:
                is_valid_time = False

            if not is_valid_time:
                raise HTTPException(
                    status_code=400,
                    detail=f"예약주문 접수 가능 시간이 아닙니다. 현재 시간: {now_kst.strftime('%H:%M:%S')} (KST). "
                           f"접수 가능 시간: 10:00~23:20 (16:30~16:45 제외)"
                )
        ...
        if result.get("rt_cd") != "0":
            ...
            error_msg = result.get("msg1", "주문 접수 실패")
            if "예약주문접수가능시간이 아닙니다" in error_msg:
                error_msg = f"예약주문 접수 가능 시간이 아닙니다. (현재: {now_kst.strftime('%H:%M:%S')} KST, 접수 가능: 10:00~23:20, 16:30~16:45 제외)"
...
class OrderOverseasRequest(BaseModel):
    pdno: str = "AAPL"  # 종목 코드 (예: AAPL)
    ovrs_excg_cd: str = "NASD"  # 거래소 코드 (예: NASD - 나스닥)
    ord_qty: str = "1"  # 주문 수량 (예: 1)
    ovrs_ord_unpr: str = "180.00"  # 주문 단가 (예: 180.00)
    is_buy: bool = True  # 매수 여부 (True: 매수, False: 매도)
